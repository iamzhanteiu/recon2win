...
| `utils.py` | `validate_domain`, `make_result` (contract chuẩn), IO helpers (`read_lines`/`write_lines`/`write_json`/`write_jsonl`/`read_jsonl`), `create_output_structure` |
...
| `fuzz_depth.py` | Tier deep/standard/light + chọn wordlist theo tech xác định (httpx `-td` + `tech_confirmed.json` tích luỹ qua các lần scan) | — |
...
| `apidocs.py` | Probe OpenAPI/Swagger + OSINT (Postman/GitHub); `build_candidates()` cũng được `misconfig_probe.py` tái dùng |
...
| `dashboard.py` | `outputs/dashboard.html` — tổng quan mọi target, đọc lại `logs/stages.json` | `report` (`classify_stages`, `load_json_safe`, `missing_tools_from_skips`, `rel_link`) |
...
fuzz_depth.py  ←── dirsearch.py, ffuf.py, misconfig_probe.py
